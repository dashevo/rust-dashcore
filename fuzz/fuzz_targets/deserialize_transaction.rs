...
fn do_test(data: &[u8]) {
    let tx_result: Result<dash::blockdata::transaction::Transaction, _> = dash::consensus::encode::deserialize(data);
    match tx_result {
        Err(_) => {},
        Ok(mut tx) => {
            let ser = dash::consensus::encode::serialize(&tx);
            assert_eq!(&ser[..], data);
            let len = ser.len();
            let calculated_weight = tx.get_weight();
            for input in &mut tx.input {
                input.witness = dash::blockdata::witness::Witness::default();
            }
            let no_witness_len = dash::consensus::encode::serialize(&tx).len();
